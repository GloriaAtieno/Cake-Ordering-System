<?php
    ...
    if(isset($_GET['cake'])==false || empty($_GET['cake'])==true)
    {
        header("Location: cake_reports.php");
        exit();
    }
    else
    {
        $sql00 ="SELECT `id` from `products` WHERE `id`=?";
        $stmt00= $con->prepare($sql00);
        $stmt00->bind_param('s',$_GET['cake']);
        $stmt00->execute();
...
<html lang="en">
	<head>
		...
		<title>Glowreea's Confectionery Corner | Cake purchase Report</title>
		<!-- Font awesome -->
		...
		<div class="">
			<div class="content-wrapper">
				<div class="container-fluid">
					<div class="row">
						<div class="col-md-12">
							<h2 class="page-title">Cake purchase report</h2>
							<div class="panel panel-default">
								<div class="panel-heading">
									<caption>CAKE REPORTS FOR 
                                        <?php
								...
								<div class="panel-body">
									<table id="zctb" class="table table-striped table-responsive table-responsive-sm table-responsive-md table-responsive-lg table-responsive-xl" cellspacing="0" width="100%">
										...
										<tbody>
											<?php
												$sql02 = "SELECT * FROM `products` WHERE `id`=?";
												// Prepare the SQL SELECT statement
												$stmt02 = $con->prepare($sql02);
                                                // Bind the one parameter to the statement
                                                $stmt02->bind_param('s',$_GET['cake']);
												// Execute the statement
												$stmt02->execute();
												// Retrieve the result set
