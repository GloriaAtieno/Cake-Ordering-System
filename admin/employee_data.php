...
<body>
		...
		<div class="content">
			...
			if(isset($_GET['aksi']) == 'delete'){
				...
				if(mysqli_num_rows($cek) == 0){
					...
					$delete = mysqli_query($con, "DELETE FROM employees WHERE national_id='$national_id'");
					if($delete){
			...
			<table class="table table-striped table-hover">
				...
				<?php
				if($filter){
					$sql = mysqli_query($con, "SELECT * FROM employees WHERE status='$filter' ORDER BY national_id ASC");
				}else{
					$sql = mysqli_query($con, "SELECT * FROM employees ORDER BY national_id ASC");
				}
	...
	<p>&copy; Glowreea's Confectionery Corner 2024</p>
		</center>
