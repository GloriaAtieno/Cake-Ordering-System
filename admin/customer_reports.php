...
<html lang="en">
	<head>
		...
		<title>Glowreea's Confectionery Corner | Customer Report</title>
		<!-- Font awesome -->
						...
						<div class="col-md-12">
							...
							<div class="panel panel-default">
								<div class="panel-heading">
									<caption>CUSTOMER REPORTS</caption>
								</div>
								<div class="panel-body">
									...
									<table id="zctb" class="table table-striped table-responsive table-responsive-sm table-responsive-md table-responsive-lg table-responsive-xl" cellspacing="0" width="100%">
										...
										<tbody>
											<?php
												$sql00 = "SELECT * FROM `orders` GROUP BY `user_id`";
											...
											?>
												<tr>
													...
													<?php
														// SQL SELECT statement with one parameter placeholders
														$sql02 = "SELECT `orderDate` FROM `orders` WHERE `user_id`=? ORDER BY `orderDate` DESC";
														// Prepare the SQL SELECT statement
														$stmt02 = $con->prepare($sql02);
														// Bind the one parameter to the statement
														$stmt02->bind_param('i', $row00['user_id']);
														// Execute the statement
														$stmt02->execute();
														// Retrieve the result set
													...
													<?php
														// SQL SELECT statement with one parameter placeholders
														$sql03 = "SELECT SUM(total) AS `total` FROM `orders` WHERE `user_id`=?";
														// Prepare the SQL SELECT statement
														$stmt03 = $con->prepare($sql03);
														// Bind the one parameter to the statement
														$stmt03->bind_param('i', $row00['user_id']);
														// Execute the statement
														$stmt03->execute();
														// Retrieve the result set
													...
													<td>
														<a type="button" href="customer_order_details.php?cust=<?php echo $row00['user_id']; ?>" class="btn btn-primary btn-sm" style="background-color: blue;">
															View
														</a>
													</td>
												</tr>
